...
Conceptually yes, transcoding is just chaining a decode process to an encode process, but as we discussed earlier, a `VideoEncoder` and `VideoDecoder` aren't simple `async` calls, but rather more like [Rube Goldberg machines](../../reference/inside-jokes#rube-goldberg-machine) that you have to push chunks and frames through.
