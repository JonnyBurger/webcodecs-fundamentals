...
Media over Quic greatly simplifies the networking aspect (you don't even need to host your own server!) while also being performant and providing low-level control over how and when you send encoded video chunks.

This makes it ideal for our use case of streaming encoded chunks, so that's what we'll use in the rest of our examples.  You are of course free to use any data transfer mechanism you see fit, that is one of the benefits of WebCodecs.
