...
A primary use case for streaming from a browser to a server would be for "recording" features, such as preserving a meeting recording, or recording high-definition versions of user's web-cameras, which are common features of meeting, webinar and live-streaming softwares.

Here we'll have a server connect to a MoQ relay, and subscribe to the same stream that subscribers listen to.

![](/assets/patterns/livestreaming/live-streaming-recording.svg)


Many existing platforms use a similar approach with WebRTC where the sever joins a session as a participant and just 'records', usually involving decoding the WebRTC stream, and re-encoding it to a file.
...
On the server, for each file you record you'd create a new output with the file you want to write to 
...
```
Where `frame` and `meta` are essentially what come out from the `AudioEncoder` and `VideoEncoder`, but because we are transmitting data via binary, we need to encode it to binary on the client, and decode it to binary on the server.
...
For streaming encoded video to the browser, we'll use WebSockets for now. You'll also notice I didn't quite use the Hang format here in the serialization, I changed it a bit to make it easier to work with WebSockets and MediaBunny.
