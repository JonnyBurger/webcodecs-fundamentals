...
Then we can create a pipeline. The AbortController allows us to stop the pipelines at any time via `abortController.abort()`, and so you could create a  'stop recording' button and have that call `abortController.abort()`  to stop recording.
