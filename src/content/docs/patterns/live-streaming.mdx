...
But then you'd be sending 30 http requests per second, which is slow and error prone. There's also no way to send data back to the client. There are optimizations you could make, and you could make this work if you just need to upload media in chunks without worrying about real-time delivery, but there are also inherently more suitable options.
