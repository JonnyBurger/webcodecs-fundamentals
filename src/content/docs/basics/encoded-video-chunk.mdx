...
But if the video has "B" frames, then the frame needs to reference information from *future* frames. Video Decoders can't magically read the future [[citation needed]](../../reference/easter-eggs#citation-needed) so we instead need to decode chunks in a different order from which we display them.
